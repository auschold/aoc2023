...
fun main() {
    ...
    val checksum1 = universe.rawGalaxyCoordinates
        ...
        }.map { universe.getDistanceBetween(it.first, it.second) }
        .sum()
...
private data class Universe(
    ...
    fun getDistanceBetween(a: Coordinate, b: Coordinate): Long {
        val distX = IntRange.from(a.x, b.x).sumOf { if (isColumnEmpty(it)) 2L else 1L } - 1
        val distY = IntRange.from(a.y, b.y).sumOf { if (isRowEmpty(it)) 2L else 1L } - 1
